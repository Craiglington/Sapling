...
describe("Router", () => {
    ...
    it("should init the RouterService and redirect", () => {
        ...
        const insertSpy = spyOn(RouterService, "insert").and.callThrough();
        RouterService.init({
            routes: [
                {
                    path: new RegExp(/^\/?$/),
        ...
        expect(history.pushState.calls.mostRecent().args[2]).toBe("/one");
    });
    ...
    it("should not allow routing if no 'app-router' element has been found", () => {
        ...
        }).toThrow(new Error("The 'app-router' element has not been added to a template."));
    ...
    it("should insert notFound component with no match", () => {
        ...
        expect(history.pushState).toHaveBeenCalledTimes(1);
        expect(history.pushState.calls.mostRecent().args[2]).toBe("/");
    });
    it("should insert correct component with match", () => {
        ...
        expect(history.pushState).toHaveBeenCalledTimes(1);
        expect(history.pushState.calls.mostRecent().args[2]).toBe("/two");
    });
