...
rm -rf ../template/.simple-web-client/nginx/src/**/*.js
if ! cp -r dist/* ../template/.simple-web-client/nginx/src/; then
    echo -e "\n${RED}-> Failed to transfer src${RESET}"
    exit 1
fi

echo -e "\n${GREEN}-> Successfully transferred nginx to template${RESET}"